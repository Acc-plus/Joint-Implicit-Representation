...
class MultiSDFEX(nn.Module):
    ...
    glyph_dim=32):
        ...
        self.sdf_net2 = modules.SingleBVPNet(type='relu',mode='mlp', 
                                          hidden_features=hidden_num, 
                                          num_hidden_layers=num_hidden_layer, 
                                          in_features=hidden_num,
                                          out_features=1)

        self.hyper_sdf_net2 = HyperNetwork(hyper_in_features=self.latent_dim, 
                                          hyper_hidden_layers=hyper_hidden_layers, hyper_hidden_features=hyper_hidden_features, 
                                          hypo_module=self.sdf_net2)
    ...
    def forward(self, model_input):
        ...
        hypo_sdf2= self.hyper_sdf_net2(sdf_embedding)
        ...
        for key in hypo_sdf2.keys():
            hypos2[key] = hypo_sdf2[key] + hypo_glyph2[key]
        
        output_mid = self.sdf_net(coords_sdf, params=hypos)
        output_sdf = self.sdf_net2({'coords': output_mid['model_out']}, params=hypos2)
